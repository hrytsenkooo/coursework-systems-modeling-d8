...
namespace coursework.Experiments
{
    public static class SensitivityAnalysis
    {
        ...
        public static void RunAll()
        {
            ...
            {
                if (current_s == _baseCfg.s) continue; 
            ...
            ExperimentRunner.RunSeries($"Policy (s,Q)=(9,3)", cfg_sQopt_3);
            ...
            Console.WriteLine($"--- (Runtime={extendedRunTime} min, Reps={extendedReps}) ---");
            ...
            ExperimentRunner.RunSeries("Load: Low Demand (-20%)", _baseCfg with { StockMeanTime = 1200 * 1.2 }); 
            ExperimentRunner.RunSeries("Load: High Demand (+20%)", _baseCfg with { StockMeanTime = 1200 * 0.8 }); 

            ExperimentRunner.RunSeries("Bottleneck: Admin Delay +50%", _baseCfg with { AdminDelay = 60 * 1.5 });

            ExperimentRunner.RunSeries("Bottleneck: Kitting Time +50%", _baseCfg with
            {
                KittingUniform = (_baseCfg.KittingUniform.Item1 * 1.5, _baseCfg.KittingUniform.Item2 * 1.5)
            });
        }
    }
}